...
oscPathV3_4_0{3,2} = 's';
%oscPathV3_4_0{4,1} = '/muse/elements/gamma_relative';
%oscPathV3_4_0{4,2} = 'ffff';
...
% {'TP9'; 'FP1'; 'FP2'; 'TP10'}
system(['start "Running: muse-io.exe --preset 14 --device Muse-69D0" "C:\Program Files (x86)\Muse\muse-io.exe" --preset 14 --osc osc.tcp://localhost:' num2str(port)]);


% This flag (tcpFlag) indicates if the TCP connection will be done using the 
% TCP/IP objects(tcpFlag == true) Instrumentation Control Toolbox and Release >2011 are needed; 
% OR using 
...
if tcpFlag
    ...
    serverSSocket.setSoTimeout(timeoutSec*10000);
...
%gammaName='gamma relative';
%gammaBuffer= zeros([10*secBuffer,4]);
...
%            conf1 = false;
%          end
       % case oscPathV3_4_0{4,1}
       %  gammaBuffer= [gammaBuffer(2:end, :); cell2mat(data)];
        otherwise
        %Do nothing    
...
%Plot every 44 EEG samples approx 200ms
    ...
    if eegCounter == 50%beats*30               % - threshhold)/(maximum-threshold)                %%SS edit
%         if plot1
%          subplot(3,1,1);
...
%          
%          %subplot(3,1,3);
%          %time=0:1/10:secBuffer-1/10;
%          %h3=plot(time,gammaBuffer);
%          %xlabel('Time (s)')
%          %ylabel('Gamma relative power');
%         
%          plot1 = false;
%         
%         else
...
%          set(h2,{'ydata'},cell2);
%          %cell3= (num2cell(gammaBuffer,1))';
%          %set(h3,{'ydata'},cell3);
%         end
  %  drawnow;   
    [beat, note, vol]= soundData(eegBuffer(end,:),8*rand(1));    %%SS function              
    beats = Note_Function([note,vol,beat]);            %%HP function
    eegCounter = 0;
    end % if eegCounter   
end %while true
